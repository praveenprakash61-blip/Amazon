...
- Name: test1.spec.js >> Login test >> login test for standard_user
- Location: tests/test1.spec.js:11:9
...
Call log:
  - Expect "toHaveURL" with timeout 50000ms
    54 × unexpected value "https://www.saucedemo.com/inventory.html"
...
```ts
  ...
  2  | 
  3  | const users = [
  4  |   { username: "standard_user", password: "secret_sauce", valid: true },
  5  |   { username: "locked_out_user", password: "secret_sauce", valid: false },
  6  |   { username: "problem_user", password: "secret_sauce", valid: false },
  7  | ];
  8  | 
  9  | test.describe("Login test", () => {
  10 |   users.forEach((user) => {
  11 |     test(`login test for ${user.username}`, async ({ page }) => {
  12 |       await page.goto("https://www.saucedemo.com/");
  13 |       await page.fill("#user-name", user.username);
  14 |       await page.fill("#password", user.password);
  15 |       await page.click("#login-button");
  16 | 
  17 |       if (user.valid) {
> 18 |         await expect(page).toHaveURL("/inventory/");
     |                            ^ Error: expect(page).toHaveURL(expected) failed
  19 |         await expect(page.locator(".ttile")).toHaveText("Products");
  20 |       } else {
